...
const mockClientData = {
  ...
  reports: [ { id: 1, date: '2026-04-10', leads: 120, cost: '4.50', contracts: 8, custom: [], name: 'Fechamento Teste' } ],
...
export default function CentralApp() {
  ...
  const syncClient = async (client) => {
    ...
    try {
      ...
      if (errorMessage === 'Failed to fetch' || errorMessage.includes('fetch')) {
          errorMessage = 'Servidor offline, link incorreto ou bloqueio de segurança (HTTPS exigindo HTTP).';
      }
      setClients(prev => safeArray(prev).map(c => c.id === client.id ? { ...c, status: 'offline', error: errorMessage } : c));
      showToast(`Erro em ${client.name}: ${errorMessage}`);
    } finally {
      setSyncingId(null);
    }
  };
  ...
  const saveClientData = async (client, endpoint, newData) => {
    ...
    try {
      ...
      let errorMessage = err.message;
      if (errorMessage === 'Failed to fetch' || errorMessage.includes('fetch')) {
          errorMessage = 'Servidor offline ou bloqueio de HTTP.';
      }
                    ...
                    <div className="p-5 border-b border-gray-800 flex justify-between items-start">
                      <div>
                        ...
                        <div className="flex flex-col items-start mt-1 gap-1">
                          <a href={client.url} target="_blank" rel="noreferrer" className="text-xs text-blue-400 hover:text-blue-300 flex items-center gap-1 font-mono">
                            {client.url} <ExternalLink size={10} />
                          </a>
                          {isMaster && client.whatsapp && (
                            <a href={client.whatsapp} target="_blank" rel="noreferrer" className="text-xs text-emerald-400 hover:text-emerald-300 flex items-center gap-1 font-mono">
                              Ver Leads (WhatsApp) <ExternalLink size={10} />
                            </a>
                          )}
                        </div>
                      </div>
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="rounded-3xl w-full max-w-5xl border border-gray-700 shadow-[0_0_50px_rgba(0,0,0,0.5)] flex flex-col max-h-[90vh] overflow-hidden" style={{ backgroundColor: safeConfig.cardBgColor }}>
            ...
            <div className="p-6 border-b border-gray-800 flex flex-col md:flex-row md:justify-between md:items-center gap-4 bg-black/30 flex-shrink-0">
              <div>
                ...
                <div className="flex flex-col gap-1 mt-1">
                  <a href={activeClient.url} target="_blank" rel="noreferrer" className="text-sm text-gray-500 hover:text-gray-300 inline-flex items-center gap-1 font-mono">
                    {activeClient.url} <ExternalLink size={12}/>
                  </a>
                  {isMaster && activeClient.whatsapp && (
                    <a href={activeClient.whatsapp} target="_blank" rel="noreferrer" className="text-sm text-emerald-500 hover:text-emerald-400 inline-flex items-center gap-1 font-mono">
                      Ver Leads (WhatsApp) <ExternalLink size={12}/>
                    </a>
                  )}
                </div>
              </div>
              <div className="flex flex-wrap gap-2">
                ...
                {isMaster && (
                  <button onClick={() => setActiveClientTab('traffic')} className={`px-4 py-2 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${activeClientTab === 'traffic' ? 'text-white shadow-lg' : 'bg-black/40 text-gray-400 hover:bg-black/60'}`} style={activeClientTab === 'traffic' ? primaryBg : {}}>
                    <TrendingUp size={16}/> Tráfego & BI
                  </button>
                )}
            ...
            <div className="p-6 overflow-y-auto flex-1 custom-scrollbar space-y-8 relative">
              ...
              {/* ABA: TRÁFEGO E BI (RESTAURADA) */}
              {activeClientTab === 'traffic' && isMaster && (
                <div className="space-y-8">
                  <section>
                    ...
                    {(!activeClient.data?.reports || safeArray(activeClient.data.reports).length === 0) ? (
                      <div className="text-center py-8 bg-black/20 rounded-2xl border border-gray-800 text-gray-500 font-bold">Nenhum relatório na VPS deste cliente.</div>
                    ) : (
                      <div className="space-y-4">
                        {safeArray(activeClient.data.reports).slice(0, 3).map(rep => (
                          <div key={rep.id} className="bg-black/40 p-5 rounded-2xl border border-gray-700 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                            <div>
                              <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-1">Referência: {rep.month || formatSafeDate(rep.date)}</p>
                              <h4 className="text-lg font-bold text-white mb-2">{rep.name || 'Fechamento de Performance'}</h4>
                  ...
                  {activeClient.data?.config?.lookerStudioUrl && (
                    <section>
                      <div className="flex items-center gap-2 border-b border-gray-800 pb-3 mb-4">
                        <BarChart3 className="text-blue-400"/>
                        <h3 className="text-lg font-black text-white uppercase tracking-widest">Dashboard Data Studio</h3>
                      </div>
        ...
        <div className="fixed inset-0 bg-black/95 backdrop-blur-md z-[60] flex items-center justify-center p-4">
          <div className="bg-[#111827] rounded-3xl p-8 w-full max-w-md border border-gray-700 shadow-2xl">
            ...
            <form onSubmit={(e) => {
              ...
              <div>
                <label className="text-xs font-bold text-gray-400 uppercase mb-1 block">Link do WhatsApp (Leads)</label>
                <input name="whatsapp" defaultValue={editingClient?.whatsapp || ''} placeholder="https://wa.me/5511999999999" className="w-full p-3 bg-[#1F2937] border border-gray-700 rounded-xl outline-none focus:border-cyan-500 text-white font-mono text-sm" />
                <p className="text-[10px] text-gray-500 mt-1">Apenas Master e Super verão este link.</p>
              </div>
